...
const PROBLEMS = [
  ...
  },
  {
    ...
    level: "intermediate",
    category: "engineering",
    title: "【pandas】DataFrameを作って中身を確認（shape・columns）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\n辞書 data から pandas の DataFrame を作り、行数・列数（shape）と列名の一覧（columns）を出力してください。\n\n期待される出力:\n(3, 3)\n['name', 'age', 'city']",
    starterCode: "import pandas as pd\n\ndata = {\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\"],\n    \"age\": [20, 25, 32],\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\"],\n}\n# data から DataFrame を作り、df.shape と list(df.columns) を出力してください\n",
    expectedOutput: "(3, 3)\n['name', 'age', 'city']",
    explanation: "正解例:\n\nimport pandas as pd\n\ndata = {\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\"],\n    \"age\": [20, 25, 32],\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\"],\n}\ndf = pd.DataFrame(data)\nprint(df.shape)\nprint(list(df.columns))\n\npd.DataFrame(辞書)で、辞書のキーが列名、値のリストが各列のデータになるDataFrameを作れます。df.shapeは(行数, 列数)のタプル、df.columnsは列名の一覧（Indexオブジェクト）です。list()で囲むことで通常のリストとして表示され、pandasのバージョンによる表示形式の違いを避けられます。SQLで言えばCREATE TABLE＋INSERTでテーブルを作るのに近い操作です。\n\nよくある間違い:\n・df.shapeを関数のように df.shape() と書いてしまう（shapeはメソッドではなく属性なので()は不要）\n・df.columnsをそのままprintすると Index(['name', 'age', 'city'], dtype='object') のような表示になり見た目が変わる（list()で囲むと安定した表示になる）",
    hint: "df = pd.DataFrame(data) でDataFrameを作り、print(df.shape) と print(list(df.columns)) を書きます。",
  ...
  },
  {
    ...
    level: "intermediate",
    category: "engineering",
    title: "【pandas】CSV文字列をread_csvで読む（io.StringIO）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\n変数 csv_text はCSV形式の文字列です。io.StringIOでファイルのように扱い、pd.read_csvでDataFrameとして読み込んで、列名の一覧と price 列の合計を出力してください。\n\n期待される出力:\n['name', 'price']\n1700",
    starterCode: "import pandas as pd\nimport io\n\ncsv_text = \"name,price\\nりんご,120\\nバナナ,80\\nメロン,1500\"\n# io.StringIO(csv_text) を pd.read_csv に渡して読み込んでください\n",
    expectedOutput: "['name', 'price']\n1700",
    explanation: "正解例:\n\nimport pandas as pd\nimport io\n\ncsv_text = \"name,price\\nりんご,120\\nバナナ,80\\nメロン,1500\"\ndf = pd.read_csv(io.StringIO(csv_text))\nprint(list(df.columns))\nprint(int(df[\"price\"].sum()))\n\npd.read_csv()は本来ファイルパスを渡してCSVファイルを読み込む関数ですが、io.StringIO(文字列)で文字列を「ファイルのように見せかける」ことで、ファイルを作らずにCSVの読み込みをテストできます。実務ではAPIレスポンスやログから取得した文字列をその場でDataFrameに変換したいときによく使うテクニックです。df[\"price\"].sum()の結果はnumpyの整数型なので、int()で明示的にPythonのintに変換してから出力しています。\n\nよくある間違い:\n・io.StringIOを使わず文字列を直接pd.read_csvに渡してしまいエラーになる\n・sum()の結果をint()で変換せずそのままprintすると、pandasのバージョンによって表示が変わることがある（今回はint()で変換しているため安定する）",
    hint: "df = pd.read_csv(io.StringIO(csv_text)) で読み込み、list(df.columns) と int(df[\"price\"].sum()) を出力します。",
    answerCode: "import pandas as pd\nimport io\n\ncsv_text = \"name,price\\nりんご,120\\nバナナ,80\\nメロン,1500\"\ndf = pd.read_csv(io.StringIO(csv_text))\nprint(list(df.columns))\nprint(int(df[\"price\"].sum()))"
  },
  {
    id: "i086",
    level: "intermediate",
    category: "engineering",
    title: "【pandas】列の選択と条件抽出（ブールインデックス）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df から、age が25以上の行だけを抽出し、name列の値をリストとして出力してください。\n\n期待される出力:\n['Hanako', 'Jiro', 'Sachiko']",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\", \"Sachiko\"],\n    \"age\": [20, 25, 32, 28],\n})\n# df[\"age\"] >= 25 の条件で行を絞り込み、name列を取り出してください\n",
    expectedOutput: "['Hanako', 'Jiro', 'Sachiko']",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\", \"Sachiko\"],\n    \"age\": [20, 25, 32, 28],\n})\nfiltered = df[df[\"age\"] >= 25][\"name\"]\nprint(filtered.tolist())\n\ndf[\"age\"] >= 25は各行についてTrue/Falseを返すSeries（ブール配列）です。これをdf[...]に渡すと、Trueの行だけが残ります。これを「ブールインデックス」と呼びます。続けて[\"name\"]で1列だけ取り出し、tolist()で通常のPythonリストに変換しています。SQLで言えばWHERE age >= 25に相当する処理です。\n\nよくある間違い:\n・df[\"age\"] >= 25 の結果を if文で使おうとしてエラーになる（Seriesは1つのbool値ではなく行ごとの配列なので、if文の条件には使えない）\n・tolist()を忘れてSeriesのままprintすると、インデックス番号や dtype の行も一緒に表示されてしまう",
    hint: "df[df[\"age\"] >= 25][\"name\"] で条件に合う行のname列を取り出せます。最後に.tolist()でリストに変換します。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\", \"Sachiko\"],\n    \"age\": [20, 25, 32, 28],\n})\nfiltered = df[df[\"age\"] >= 25][\"name\"]\nprint(filtered.tolist())"
  },
  {
    id: "i087",
    level: "intermediate",
    category: "engineering",
    title: "【pandas】新しい列を作る（列同士の計算）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df の price列とqty列を掛け算した subtotal列を新しく作り、その値をリストとして出力してください。\n\n期待される出力:\n[360, 400, 1500]",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"item\": [\"りんご\", \"バナナ\", \"メロン\"],\n    \"price\": [120, 80, 1500],\n    \"qty\": [3, 5, 1],\n})\n# df.assign(subtotal=...) で price * qty の新しい列を作ってください\n",
    expectedOutput: "[360, 400, 1500]",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"item\": [\"りんご\", \"バナナ\", \"メロン\"],\n    \"price\": [120, 80, 1500],\n    \"qty\": [3, 5, 1],\n})\ndf = df.assign(subtotal=df[\"price\"] * df[\"qty\"])\nprint(df[\"subtotal\"].tolist())\n\n列同士の計算は+ - * /のような通常の演算子をそのまま使え、行ごとに自動で計算されます。df.assign(新しい列名=計算式)は「新しい列を追加したDataFrameを返す」書き方で、df[\"subtotal\"] = ... という直接代入と同じ結果になりますが、コード上で「元のdfを新しいdfに置き換えている」ことが分かりやすく、意図しない書き換えのミスを防ぎやすい書き方です。SQLで言えばSELECT price * qty AS subtotalに相当します。\n\nよくある間違い:\n・df.assign(subtotal=df[\"price\"] * df[\"qty\"])の結果を変数に代入し忘れ、dfが更新されない\n・列名のスペルミス（\"price\"と\"prise\"など）でKeyErrorになる",
    hint: "df = df.assign(subtotal=df[\"price\"] * df[\"qty\"]) で新しい列を作り、df[\"subtotal\"].tolist() を出力します。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"item\": [\"りんご\", \"バナナ\", \"メロン\"],\n    \"price\": [120, 80, 1500],\n    \"qty\": [3, 5, 1],\n})\ndf = df.assign(subtotal=df[\"price\"] * df[\"qty\"])\nprint(df[\"subtotal\"].tolist())"
  },
  {
    id: "i088",
    level: "intermediate",
    category: "engineering",
    title: "【pandas】sort_valuesで並べ替え",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df を score列の大きい順に並べ替え、name列の値をリストとして出力してください。\n\n期待される出力:\n['Bチーム', 'Dチーム', 'Aチーム', 'Cチーム']",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Aチーム\", \"Bチーム\", \"Cチーム\", \"Dチーム\"],\n    \"score\": [82, 95, 78, 90],\n})\n# df.sort_values(\"score\", ascending=False) で並べ替えてください\n",
    expectedOutput: "['Bチーム', 'Dチーム', 'Aチーム', 'Cチーム']",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Aチーム\", \"Bチーム\", \"Cチーム\", \"Dチーム\"],\n    \"score\": [82, 95, 78, 90],\n})\nsorted_df = df.sort_values(\"score\", ascending=False)\nprint(sorted_df[\"name\"].tolist())\n\ndf.sort_values(列名)は指定した列の値で行を並べ替えた新しいDataFrameを返します。デフォルトは昇順（小さい順）なので、大きい順にしたい場合はascending=Falseを指定します。元のdfは変更されず、sorted_dfという新しい変数に結果を受け取っています。SQLで言えばORDER BY score DESCに相当する操作です。\n\nよくある間違い:\n・ascending=Falseを付け忘れて昇順のまま出力してしまう\n・sort_values()がリスト標準のsort()のように「元のリストを直接書き換える」と誤解し、df.sort_values(\"score\")だけ呼んで結果を受け取らずに使ってしまう（sort_valuesは新しいDataFrameを返すだけで、元のdfは変わらない）",
    hint: "sorted_df = df.sort_values(\"score\", ascending=False) で並べ替え、sorted_df[\"name\"].tolist() を出力します。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"name\": [\"Aチーム\", \"Bチーム\", \"Cチーム\", \"Dチーム\"],\n    \"score\": [82, 95, 78, 90],\n})\nsorted_df = df.sort_values(\"score\", ascending=False)\nprint(sorted_df[\"name\"].tolist())"
  },
  {
    id: "i089",
    level: "intermediate",
    category: "engineering",
    title: "【pandas】value_countsでカテゴリ集計",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df のcity列について、値ごとの出現回数を集計し、辞書として出力してください。\n\n期待される出力:\n{'Tokyo': 3, 'Osaka': 2, 'Nagoya': 1}",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Tokyo\", \"Osaka\", \"Nagoya\"],\n})\n# df[\"city\"].value_counts() で出現回数を集計し、to_dict() で辞書にしてください\n",
    expectedOutput: "{'Tokyo': 3, 'Osaka': 2, 'Nagoya': 1}",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Tokyo\", \"Osaka\", \"Nagoya\"],\n})\ncounts = df[\"city\"].value_counts()\nprint(counts.to_dict())\n\nvalue_counts()は「列の中の値ごとの出現回数」を、多い順に並んだSeriesとして返します。素のPythonのcollections.Counterと似た働きですが、pandasの列（Series）に対して直接呼べる点が便利です。to_dict()でPythonの標準的な辞書に変換すると、表示形式がpandasのバージョンに左右されず安定します。\n\nよくある間違い:\n・to_dict()を付けずにvalue_counts()の結果をそのままprintすると、Series特有の表示（列名やLength、dtypeの行）が付いてしまう\n・groupby(\"city\").size()でも同じ集計はできるが、value_countsは自動で多い順に並ぶのに対しgroupbyはキーの昇順になる、という違いがある",
    hint: "df[\"city\"].value_counts().to_dict() で「値ごとの出現回数」の辞書が得られます。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Tokyo\", \"Osaka\", \"Nagoya\"],\n})\ncounts = df[\"city\"].value_counts()\nprint(counts.to_dict())"
  },
  {
    id: "i090",
    level: "intermediate",
    category: "engineering",
    title: "【pandas】fillnaで欠損値を埋める",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df のsales列には欠損値（None）が含まれています。fillnaを使って欠損値を0で埋め、sales列の値をリストとして出力してください。\n\n期待される出力:\n[100.0, 0.0, 300.0]",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"day\": [\"Mon\", \"Tue\", \"Wed\"],\n    \"sales\": [100, None, 300],\n})\n# df[\"sales\"].fillna(0) で欠損値を0に置き換えてください\n",
    expectedOutput: "[100.0, 0.0, 300.0]",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"day\": [\"Mon\", \"Tue\", \"Wed\"],\n    \"sales\": [100, None, 300],\n})\ndf = df.assign(sales=df[\"sales\"].fillna(0))\nprint(df[\"sales\"].tolist())\n\nCSVやAPIレスポンスから読み込んだデータには欠損値（値が無い部分）がよく含まれ、pandasではNaN（Not a Number）として扱われます。fillna(値)は欠損値をすべて指定した値に置き換えた列を返します。列にNoneやNaNが1つでも含まれると、その列全体がfloat型（小数）になるため、0を埋めた後も0.0のように表示される点に注意してください。SQLで言えばCOALESCE(sales, 0)に相当する処理です。\n\nよくある間違い:\n・fillna(0)の結果を代入し忘れ、元のsales列がNaNのまま残ってしまう\n・欠損があるとint型ではなくfloat型になることを知らず、出力が100ではなく100.0になるのを見て間違いだと勘違いする",
    hint: "df = df.assign(sales=df[\"sales\"].fillna(0)) で欠損値を埋め、df[\"sales\"].tolist() を出力します。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"day\": [\"Mon\", \"Tue\", \"Wed\"],\n    \"sales\": [100, None, 300],\n})\ndf = df.assign(sales=df[\"sales\"].fillna(0))\nprint(df[\"sales\"].tolist())"
  },
  {
    id: "a072",
    level: "advanced",
    category: "engineering",
    title: "【pandas】groupby + 集計（sum/mean）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df をcity列でグループ化し、それぞれのグループのsales列の合計を辞書として出力してください。\n\n期待される出力:\n{'Osaka': 600, 'Tokyo': 400}",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"sales\": [100, 200, 300, 400],\n})\n# df.groupby(\"city\")[\"sales\"].sum() で都市ごとの合計を求めてください\n",
    expectedOutput: "{'Osaka': 600, 'Tokyo': 400}",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"sales\": [100, 200, 300, 400],\n})\nresult = df.groupby(\"city\")[\"sales\"].sum()\nprint(result.to_dict())\n\ndf.groupby(\"city\")は「city列の値が同じ行同士をグループにまとめる」操作で、続けて[\"sales\"].sum()で各グループのsales列を合計します。SQLのGROUP BY city, SUM(sales)に相当する処理です。groupbyの結果はデフォルトでグループのキー（この場合は都市名）の昇順に並びます。alphabet順で\"Osaka\"が\"Tokyo\"より先に来るのはこのためです。\n\nよくある間違い:\n・df.groupby(\"city\").sum()のように列を絞らずに書くと、数値以外の列も含めて全列が集計対象になろうとしてエラーや意図しない結果になることがある\n・to_dict()を忘れてSeriesのままprintすると、pandasのバージョンで表示幅が変わる可能性がある",
    hint: "df.groupby(\"city\")[\"sales\"].sum() でグループごとの合計が求まります。to_dict()で辞書に変換して出力します。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"sales\": [100, 200, 300, 400],\n})\nresult = df.groupby(\"city\")[\"sales\"].sum()\nprint(result.to_dict())"
  },
  {
    id: "a073",
    level: "advanced",
    category: "engineering",
    title: "【pandas】aggで複数の集計を一度に",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df をcity列でグループ化し、price列の合計（sum）と平均（mean）を同時に求めて、都市ごとの結果を辞書として出力してください。\n\n期待される出力:\n{'Osaka': {'sum': 600, 'mean': 300.0}, 'Tokyo': {'sum': 400, 'mean': 200.0}}",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"price\": [100, 200, 300, 400],\n})\n# df.groupby(\"city\")[\"price\"].agg([\"sum\", \"mean\"]) で複数の集計を同時に行ってください\n",
    expectedOutput: "{'Osaka': {'sum': 600, 'mean': 300.0}, 'Tokyo': {'sum': 400, 'mean': 200.0}}",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"price\": [100, 200, 300, 400],\n})\nresult = df.groupby(\"city\")[\"price\"].agg([\"sum\", \"mean\"])\nprint(result.to_dict(orient=\"index\"))\n\nsum()やmean()を1つずつ別々に呼ぶ代わりに、agg([\"sum\", \"mean\"])と書くと、複数の集計を1回のgroupbyで同時に計算できます。結果は都市ごとに sum・mean の2列を持つDataFrameになるので、to_dict(orient=\"index\")で「行（都市）をキーにした辞書の辞書」に変換しています。SQLで言えばSELECT city, SUM(price), AVG(price) FROM t GROUP BY cityに相当します。\n\nよくある間違い:\n・to_dict()のorientを指定し忘れて既定のorient=\"dict\"（列ごとの辞書）になり、期待した「都市ごと」の形と入れ子の向きが変わってしまう\n・aggに渡す集計名をクォートし忘れて agg([sum, mean]) のように書いてしまう（文字列 \"sum\" / \"mean\" として渡す必要がある）",
    hint: "df.groupby(\"city\")[\"price\"].agg([\"sum\", \"mean\"]) で集計し、.to_dict(orient=\"index\") で辞書にします。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Osaka\", \"Tokyo\", \"Osaka\"],\n    \"price\": [100, 200, 300, 400],\n})\nresult = df.groupby(\"city\")[\"price\"].agg([\"sum\", \"mean\"])\nprint(result.to_dict(orient=\"index\"))"
  },
  {
    id: "a074",
    level: "advanced",
    category: "engineering",
    title: "【pandas】mergeでマスタと結合（SQLのJOINとの対応）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\n注文データ orders と顧客マスタ customers を customer_id をキーに結合し、結合結果を1行ずつの辞書のリストとして出力してください。\n\n期待される出力:\n[{'customer_id': 1, 'amount': 1000, 'name': 'Taro'}, {'customer_id': 2, 'amount': 2000, 'name': 'Hanako'}, {'customer_id': 1, 'amount': 1500, 'name': 'Taro'}, {'customer_id': 3, 'amount': 500, 'name': 'Jiro'}]",
    starterCode: "import pandas as pd\n\norders = pd.DataFrame({\n    \"customer_id\": [1, 2, 1, 3],\n    \"amount\": [1000, 2000, 1500, 500],\n})\ncustomers = pd.DataFrame({\n    \"customer_id\": [1, 2, 3],\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\"],\n})\n# pd.merge(orders, customers, on=\"customer_id\") で結合してください\n",
    expectedOutput: "[{'customer_id': 1, 'amount': 1000, 'name': 'Taro'}, {'customer_id': 2, 'amount': 2000, 'name': 'Hanako'}, {'customer_id': 1, 'amount': 1500, 'name': 'Taro'}, {'customer_id': 3, 'amount': 500, 'name': 'Jiro'}]",
    explanation: "正解例:\n\nimport pandas as pd\n\norders = pd.DataFrame({\n    \"customer_id\": [1, 2, 1, 3],\n    \"amount\": [1000, 2000, 1500, 500],\n})\ncustomers = pd.DataFrame({\n    \"customer_id\": [1, 2, 3],\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\"],\n})\nmerged = pd.merge(orders, customers, on=\"customer_id\")\nprint(merged.to_dict(orient=\"records\"))\n\npd.merge(左df, 右df, on=\"共通の列名\")は、2つの表を指定した列の値が一致する行同士で結合します。11節のSQLで学んだJOINとまったく同じ考え方で、on=\"customer_id\"はSQLのON orders.customer_id = customers.customer_idに相当します。デフォルトのhow=\"inner\"は両方に存在する値だけを残す（INNER JOINと同じ）動作です。to_dict(orient=\"records\")で「1行=1辞書」のリストに変換すると、行の内容が読みやすく、バージョンによる表示ゆれもありません。\n\nよくある間違い:\n・on=\"customer_id\"を書き忘れると、pandasが同名の列を自動で推測しようとしてエラーや意図しない結合になることがある\n・orders側にしかない顧客ID・customers側にしかない顧客IDがある場合、how=\"inner\"（既定）では両方に無い行が結果から消える点を忘れる",
    hint: "pd.merge(orders, customers, on=\"customer_id\") で結合し、.to_dict(orient=\"records\") で1行ずつの辞書のリストにします。",
    answerCode: "import pandas as pd\n\norders = pd.DataFrame({\n    \"customer_id\": [1, 2, 1, 3],\n    \"amount\": [1000, 2000, 1500, 500],\n})\ncustomers = pd.DataFrame({\n    \"customer_id\": [1, 2, 3],\n    \"name\": [\"Taro\", \"Hanako\", \"Jiro\"],\n})\nmerged = pd.merge(orders, customers, on=\"customer_id\")\nprint(merged.to_dict(orient=\"records\"))"
  },
  {
    id: "a075",
    level: "advanced",
    category: "engineering",
    title: "【pandas】pivot_tableでクロス集計",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nDataFrame df を、行を city・列を category として、sales列の合計をクロス集計し、辞書として出力してください。\n\n期待される出力:\n{'book': {'Osaka': 30, 'Tokyo': 50}, 'food': {'Osaka': 80, 'Tokyo': 100}}",
    starterCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Tokyo\", \"Osaka\", \"Osaka\"],\n    \"category\": [\"food\", \"book\", \"food\", \"book\"],\n    \"sales\": [100, 50, 80, 30],\n})\n# df.pivot_table(index=\"city\", columns=\"category\", values=\"sales\", aggfunc=\"sum\") でクロス集計してください\n",
    expectedOutput: "{'book': {'Osaka': 30, 'Tokyo': 50}, 'food': {'Osaka': 80, 'Tokyo': 100}}",
    explanation: "正解例:\n\nimport pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Tokyo\", \"Osaka\", \"Osaka\"],\n    \"category\": [\"food\", \"book\", \"food\", \"book\"],\n    \"sales\": [100, 50, 80, 30],\n})\npivot = df.pivot_table(index=\"city\", columns=\"category\", values=\"sales\", aggfunc=\"sum\")\nprint(pivot.to_dict())\n\npivot_tableは「1つの列を行の見出し、別の列を列の見出しにして、値を集計表（クロス集計表）にする」操作です。index=\"city\"が縦の見出し、columns=\"category\"が横の見出し、values=\"sales\"が集計する値、aggfunc=\"sum\"が集計方法（合計）です。表計算ソフトのピボットテーブルと同じ考え方です。to_dict()（既定のorient=\"dict\"）は「列（category）をキーにした辞書の辞書」を返すため、外側がbook/food、内側がOsaka/Tokyoという順番になります。\n\nよくある間違い:\n・aggfunc=\"sum\"を指定し忘れると既定のaggfunc（平均）で集計されてしまい、合計と勘違いする\n・groupbyで代用しようとして、2つの軸（city × category）を同時に表の形にする書き方が分からず複雑になってしまう（pivot_tableなら1行で書ける）",
    hint: "df.pivot_table(index=\"city\", columns=\"category\", values=\"sales\", aggfunc=\"sum\") でクロス集計し、.to_dict() で辞書にします。",
    answerCode: "import pandas as pd\n\ndf = pd.DataFrame({\n    \"city\": [\"Tokyo\", \"Tokyo\", \"Osaka\", \"Osaka\"],\n    \"category\": [\"food\", \"book\", \"food\", \"book\"],\n    \"sales\": [100, 50, 80, 30],\n})\npivot = df.pivot_table(index=\"city\", columns=\"category\", values=\"sales\", aggfunc=\"sum\")\nprint(pivot.to_dict())"
  },
  {
    id: "a076",
    level: "advanced",
    category: "engineering",
    title: "【pandas総合】CSV→クレンジング→集計→辞書出力（ミニETL）",
    description: "この問題はpandasを使います。初回実行時は読み込みに数秒かかります。\n\nCSV文字列 csv_text には、sales列に欠損値（空欄）が含まれています。①read_csvで読み込み、②fillnaで欠損値を0で埋め、③city列ごとにsalesを合計し、結果を辞書として出力してください。\n\n期待される出力:\n{'Osaka': 200, 'Tokyo': 400}",
    starterCode: "import pandas as pd\nimport io\n\ncsv_text = \"city,sales\\nTokyo,100\\nOsaka,\\nTokyo,300\\nOsaka,200\\nTokyo,\"\n# 1) read_csvで読み込む 2) sales列のfillna(0)で欠損値を埋める 3) city列でgroupbyしてsumを求めてください\n",
    expectedOutput: "{'Osaka': 200, 'Tokyo': 400}",
    explanation: "正解例:\n\nimport pandas as pd\nimport io\n\ncsv_text = \"city,sales\\nTokyo,100\\nOsaka,\\nTokyo,300\\nOsaka,200\\nTokyo,\"\ndf = pd.read_csv(io.StringIO(csv_text))\ndf = df.assign(sales=df[\"sales\"].fillna(0).astype(int))\nresult = df.groupby(\"city\")[\"sales\"].sum().to_dict()\nprint(result)\n\nこれまで学んだ「読み込み（read_csv）→クレンジング（fillna）→集計（groupby）」の3ステップを1つの処理につなげた、実務のETL（Extract・Transform・Load）処理のミニ版です。CSVの空欄は読み込み時にNaN（欠損値）になるため、集計の前にfillna(0)で埋めておく必要があります。fillna(0)だけだとfloat型のままになるため、astype(int)で整数型に戻してから合計しています。SQLであればCOALESCE(sales, 0)をかけてからGROUP BY city, SUM(sales)するのと同じ流れです。\n\nよくある間違い:\n・fillnaを忘れてgroupby().sum()すると、NaNは合計から自動的に無視されるため一見動いてしまうが、件数を数える処理（count等）では欠損行が漏れるなど別の集計で誤差の原因になる\n・astype(int)を忘れ、出力が200ではなく200.0になり期待値と一致しない",
    hint: "pd.read_csv(io.StringIO(csv_text)) → df.assign(sales=df[\"sales\"].fillna(0).astype(int)) → df.groupby(\"city\")[\"sales\"].sum().to_dict() の順に処理します。",
    answerCode: "import pandas as pd\nimport io\n\ncsv_text = \"city,sales\\nTokyo,100\\nOsaka,\\nTokyo,300\\nOsaka,200\\nTokyo,\"\ndf = pd.read_csv(io.StringIO(csv_text))\ndf = df.assign(sales=df[\"sales\"].fillna(0).astype(int))\nresult = df.groupby(\"city\")[\"sales\"].sum().to_dict()\nprint(result)"
  },
  {
    id: "a077",
    level: "advanced",
    category: "engineering",
    title: "【SQL】UPSERT（INSERT ... ON CONFLICT ... DO UPDATE）",
    description: "在庫テーブル stock（item_id・name・qty）に、新しいデータ data を「item_idが既にあれば数量を上書き、なければ新規追加」する形で反映してください。ON CONFLICT ... DO UPDATE を使います。\n\n期待される出力:\n(1, 'マグカップ', 10)\n(2, 'Tシャツ', 20)\n(3, 'ステッカー', 30)",
    starterCode: "import sqlite3\n\nconn = sqlite3.connect(\":memory:\")\ncur = conn.cursor()\ncur.execute(\"CREATE TABLE stock (item_id INTEGER PRIMARY KEY, name TEXT, qty INTEGER)\")\ncur.executemany(\"INSERT INTO stock VALUES (?, ?, ?)\", [(1, \"マグカップ\", 10), (2, \"Tシャツ\", 5)])\n\ndata = [(2, \"Tシャツ\", 20), (3, \"ステッカー\", 30)]\n# ON CONFLICT(item_id) DO UPDATE SET qty = excluded.qty を使って、\n# data を stock に反映してください（item_id=2は上書き、item_id=3は新規追加）\n",
    expectedOutput: "(1, 'マグカップ', 10)\n(2, 'Tシャツ', 20)\n(3, 'ステッカー', 30)",
    explanation: "正解例:\n\nimport sqlite3\n\nconn = sqlite3.connect(\":memory:\")\ncur = conn.cursor()\ncur.execute(\"CREATE TABLE stock (item_id INTEGER PRIMARY KEY, name TEXT, qty INTEGER)\")\ncur.executemany(\"INSERT INTO stock VALUES (?, ?, ?)\", [(1, \"マグカップ\", 10), (2, \"Tシャツ\", 5)])\n\ndata = [(2, \"Tシャツ\", 20), (3, \"ステッカー\", 30)]\ncur.executemany(\"\"\"\n    INSERT INTO stock (item_id, name, qty) VALUES (?, ?, ?)\n    ON CONFLICT(item_id) DO UPDATE SET qty = excluded.qty\n\"\"\", data)\n\nfor row in cur.execute(\"SELECT item_id, name, qty FROM stock ORDER BY item_id\"):\n    print(row)\n\nUPSERT（UPDATE + INSERTの造語）は「あれば更新・なければ追加」を1回のSQL文でまとめて行う操作です。ON CONFLICT(主キーの列名) DO UPDATE SET 列 = excluded.列 という形で書き、excluded は「今回INSERTしようとした値」を指します。実務のバッチ処理では、毎回テーブルを空にして全件入れ直す（DELETE→INSERT）方法もありますが、件数が多いと時間がかかり、処理中に他の参照が空振りするリスクもあります。UPSERTなら変更が必要な行だけを更新するため高速で、既存データを一瞬も欠かさず保てます。\n\nよくある間違い:\n・ON CONFLICTの後の(item_id)を、主キー（PRIMARY KEY）の列名ではなく別の列名にしてしまい、意図した競合判定が働かない\n・SET qty = excluded.qty の代わりに SET qty = qty と書いてしまい、値が更新されない（excludedを付け忘れる）",
    hint: "cur.executemany(\"INSERT INTO stock (item_id, name, qty) VALUES (?, ?, ?) ON CONFLICT(item_id) DO UPDATE SET qty = excluded.qty\", data) のように書きます。",
    answerCode: "import sqlite3\n\nconn = sqlite3.connect(\":memory:\")\ncur = conn.cursor()\ncur.execute(\"CREATE TABLE stock (item_id INTEGER PRIMARY KEY, name TEXT, qty INTEGER)\")\ncur.executemany(\"INSERT INTO stock VALUES (?, ?, ?)\", [(1, \"マグカップ\", 10), (2, \"Tシャツ\", 5)])\n\ndata = [(2, \"Tシャツ\", 20), (3, \"ステッカー\", 30)]\ncur.executemany(\"\"\"\n    INSERT INTO stock (item_id, name, qty) VALUES (?, ?, ?)\n    ON CONFLICT(item_id) DO UPDATE SET qty = excluded.qty\n\"\"\", data)\n\nfor row in cur.execute(\"SELECT item_id, name, qty FROM stock ORDER BY item_id\"):\n    print(row)"
  },
  {
    id: "i091",
    level: "intermediate",
    category: "engineering",
    title: "【スキーマ検証】dataclass + 型チェックで不正レコードを検出する",
    description: "dataclass Order（order_id: int・price: int・name: str）を「正しいスキーマ」として、リスト raw の各レコードの型をチェックしてください。型が一致しないフィールドがあれば「NG: order_id=X 不正フィールド=[...]」、なければ「OK: order_id=X」の形式で出力してください。\n\n期待される出力:\nOK: order_id=1\nNG: order_id=2 不正フィールド=['price']\nNG: order_id=3 不正フィールド=['name']",
    starterCode: "from dataclasses import dataclass, fields\n\n@dataclass\nclass Order:\n    order_id: int\n    price: int\n    name: str\n\nraw = [\n    {\"order_id\": 1, \"price\": 1200, \"name\": \"りんご\"},\n    {\"order_id\": 2, \"price\": \"980\", \"name\": \"バナナ\"},   # price が文字列\n    {\"order_id\": 3, \"price\": 500, \"name\": 123},          # name が数値\n]\n# fields(Order) と isinstance を使って、各レコードの型不一致フィールドを検出してください\n",
    expectedOutput: "OK: order_id=1\nNG: order_id=2 不正フィールド=['price']\nNG: order_id=3 不正フィールド=['name']",
    explanation: "正解例:\n\nfrom dataclasses import dataclass, fields\n\n@dataclass\nclass Order:\n    order_id: int\n    price: int\n    name: str\n\nraw = [\n    {\"order_id\": 1, \"price\": 1200, \"name\": \"りんご\"},\n    {\"order_id\": 2, \"price\": \"980\", \"name\": \"バナナ\"},\n    {\"order_id\": 3, \"price\": 500, \"name\": 123},\n]\n\nfor row in raw:\n    errors = [f.name for f in fields(Order) if not isinstance(row.get(f.name), f.type)]\n    if errors:\n        print(f\"NG: order_id={row.get('order_id')} 不正フィールド={errors}\")\n    else:\n        print(f\"OK: order_id={row['order_id']}\")\n\ndataclassでスキーマ（あるべきフィールド名と型）を定義しておくと、fields(クラス名)でフィールド名と型の一覧を取得できます。f.typeは各フィールドに付けた型ヒント（int・str等）そのものなので、isinstance(値, f.type)で実際の値がその型かどうかをチェックできます。CSVやAPIから取り込んだデータは型が期待通りとは限らず（数値のはずが文字列になっている等）、集計処理の手前でこのようなスキーマ検証を行うことで、後工程でのエラーや誤った集計結果を未然に防げます。例えば price が文字列 \"980\" のまま合計しようとすると、int同士の+では計算できずTypeErrorになります。\n\nよくある間違い:\n・row.get(f.name) ではなく row[f.name] を使い、キー自体が存在しないレコードでKeyErrorになってしまう（get()ならNoneが返り、isinstanceでNG判定になるだけで済む）\n・f.typeとisinstanceの組み合わせを知らず、if type(値) == f.type のように書いてしまう（isinstanceの方がサブクラスにも対応でき一般的）",
    hint: "fields(Order) で各フィールドの name と type が取れます。[f.name for f in fields(Order) if not isinstance(row.get(f.name), f.type)] で不正フィールド名のリストが得られます。",
    answerCode: "from dataclasses import dataclass, fields\n\n@dataclass\nclass Order:\n    order_id: int\n    price: int\n    name: str\n\nraw = [\n    {\"order_id\": 1, \"price\": 1200, \"name\": \"りんご\"},\n    {\"order_id\": 2, \"price\": \"980\", \"name\": \"バナナ\"},\n    {\"order_id\": 3, \"price\": 500, \"name\": 123},\n]\n\nfor row in raw:\n    errors = [f.name for f in fields(Order) if not isinstance(row.get(f.name), f.type)]\n    if errors:\n        print(f\"NG: order_id={row.get('order_id')} 不正フィールド={errors}\")\n    else:\n        print(f\"OK: order_id={row['order_id']}\")"
  },
  {
    id: "a078",
    level: "advanced",
    category: "engineering",
    title: "【スキーマ検証総合】複数フィールドの型チェック＋不正理由をまとめてレポート",
    description: "dataclass Product（product_id: int・price: int・name: str・stock: int）を正しいスキーマとして、リスト raw の各レコードを検証してください。1レコードに複数の不正フィールドがある場合はすべて理由に含め、「NG: product_id=X 理由: フィールド名は実際の型（期待:期待の型） / ...」の形式で、正しいレコードは「OK: product_id=X」で出力し、最後に「集計: OK N件 / NG M件」を出力してください。\n\n期待される出力:\nOK: product_id=1\nNG: product_id=2 理由: priceはstr型（期待:int）\nNG: product_id=3 理由: nameはint型（期待:str） / stockはstr型（期待:int）\nOK: product_id=4\n集計: OK 2件 / NG 2件",
    starterCode: "from dataclasses import dataclass, fields\n\n@dataclass\nclass Product:\n    product_id: int\n    price: int\n    name: str\n    stock: int\n\nraw = [\n    {\"product_id\": 1, \"price\": 1200, \"name\": \"りんご\", \"stock\": 50},\n    {\"product_id\": 2, \"price\": \"980\", \"name\": \"バナナ\", \"stock\": 30},\n    {\"product_id\": 3, \"price\": 500, \"name\": 123, \"stock\": \"10\"},\n    {\"product_id\": 4, \"price\": 300, \"name\": \"ぶどう\", \"stock\": 20},\n]\n# 各レコードを検証し、不正フィールドをすべて理由としてまとめ、最後に集計を出力してください\n",
    expectedOutput: "OK: product_id=1\nNG: product_id=2 理由: priceはstr型（期待:int）\nNG: product_id=3 理由: nameはint型（期待:str） / stockはstr型（期待:int）\nOK: product_id=4\n集計: OK 2件 / NG 2件",
    explanation: "正解例:\n\nfrom dataclasses import dataclass, fields\n\n@dataclass\nclass Product:\n    product_id: int\n    price: int\n    name: str\n    stock: int\n\nraw = [\n    {\"product_id\": 1, \"price\": 1200, \"name\": \"りんご\", \"stock\": 50},\n    {\"product_id\": 2, \"price\": \"980\", \"name\": \"バナナ\", \"stock\": 30},\n    {\"product_id\": 3, \"price\": 500, \"name\": 123, \"stock\": \"10\"},\n    {\"product_id\": 4, \"price\": 300, \"name\": \"ぶどう\", \"stock\": 20},\n]\n\nok_count = 0\nng_count = 0\nfor row in raw:\n    errors = []\n    for f in fields(Product):\n        value = row.get(f.name)\n        if not isinstance(value, f.type):\n            errors.append(f\"{f.name}は{type(value).__name__}型（期待:{f.type.__name__}）\")\n    if errors:\n        ng_count += 1\n        print(f\"NG: product_id={row.get('product_id')} 理由: \" + \" / \".join(errors))\n    else:\n        ok_count += 1\n        print(f\"OK: product_id={row['product_id']}\")\nprint(f\"集計: OK {ok_count}件 / NG {ng_count}件\")\n\ni091では「どのフィールドが不正か」だけを報告しましたが、実務のデータ品質レポートでは「なぜ不正なのか（実際の型と期待の型）」まで示すとエラー原因の特定が早くなります。type(value).__name__で実際の型名、f.type.__name__で期待する型名を取り出し、両方を理由の文字列に含めています。1レコードに複数の不正フィールドがある場合はerrorsリストに複数溜まるので、\" / \".join(errors)でまとめて1行にしています。最後にok_count・ng_countを集計することで、「データ全体の何％が不正か」をひと目で把握でき、データ品質のモニタリング（品質ダッシュボードの元ネタ）に直結します。\n\nよくある間違い:\n・f.type.__name__ ではなく f.type をそのままf文字列に埋め込み、<class 'int'>のような読みにくい表示になってしまう\n・複数の不正フィールドがあるレコードで、最初に見つかった1つのエラーだけでbreakしてしまい、他の不正フィールドを見逃す（今回はすべてのフィールドをループしてerrorsに溜め込む必要がある）",
    hint: "各レコードについて fields(Product) をループし、型が違うフィールドの理由文字列を errors リストに溜めます。errorsが空でなければNG、空ならOKとして集計します。",
    answerCode: "from dataclasses import dataclass, fields\n\n@dataclass\nclass Product:\n    product_id: int\n    price: int\n    name: str\n    stock: int\n\nraw = [\n    {\"product_id\": 1, \"price\": 1200, \"name\": \"りんご\", \"stock\": 50},\n    {\"product_id\": 2, \"price\": \"980\", \"name\": \"バナナ\", \"stock\": 30},\n    {\"product_id\": 3, \"price\": 500, \"name\": 123, \"stock\": \"10\"},\n    {\"product_id\": 4, \"price\": 300, \"name\": \"ぶどう\", \"stock\": 20},\n]\n\nok_count = 0\nng_count = 0\nfor row in raw:\n    errors = []\n    for f in fields(Product):\n        value = row.get(f.name)\n        if not isinstance(value, f.type):\n            errors.append(f\"{f.name}は{type(value).__name__}型（期待:{f.type.__name__}）\")\n    if errors:\n        ng_count += 1\n        print(f\"NG: product_id={row.get('product_id')} 理由: \" + \" / \".join(errors))\n    else:\n        ok_count += 1\n        print(f\"OK: product_id={row['product_id']}\")\nprint(f\"集計: OK {ok_count}件 / NG {ng_count}件\")"
  },
  {
    id: "i092",
    level: "intermediate",
    category: "engineering",
    title: "【incremental処理】前回処理したIDを状態ファイルに記録し、差分のみ処理する",
    description: "状態ファイル state.json に「最後に処理したID」を記録し、2回目の実行では前回より新しいログだけを処理するincremental処理を実装してください。1つのスクリプト内で「1回目の処理」→「状態保存」→「2回目の処理（差分のみ）」を再現します。\n\n期待される出力:\n処理: 101 A\n処理: 102 B\n処理: 103 C\n処理件数: 3\n処理: 104 D\n処理件数: 1",
    starterCode: "import json, os\n\nlogs = [{\"id\": 101, \"msg\": \"A\"}, {\"id\": 102, \"msg\": \"B\"}, {\"id\": 103, \"msg\": \"C\"}]\nSTATE_FILE = \"state.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_last_id():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)[\"last_id\"]\n    return 0\n\ndef save_last_id(last_id):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump({\"last_id\": last_id}, f)\n\n# 1) load_last_idで前回のIDを読み、logsからそれより新しいものだけ処理して\"処理: id msg\"を出力\n# 2) 処理したうち最大のIDをsave_last_idで保存し、\"処理件数: N\"を出力\n# 3) logs2 = logs + [{\"id\": 104, \"msg\": \"D\"}] に対して同じ処理をもう一度行う（前回分は再処理されないはず）\n",
    expectedOutput: "処理: 101 A\n処理: 102 B\n処理: 103 C\n処理件数: 3\n処理: 104 D\n処理件数: 1",
    explanation: "正解例:\n\nimport json, os\n\nlogs = [{\"id\": 101, \"msg\": \"A\"}, {\"id\": 102, \"msg\": \"B\"}, {\"id\": 103, \"msg\": \"C\"}]\nSTATE_FILE = \"state.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_last_id():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)[\"last_id\"]\n    return 0\n\ndef save_last_id(last_id):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump({\"last_id\": last_id}, f)\n\nlast_id = load_last_id()\nnew_logs = [l for l in logs if l[\"id\"] > last_id]\nfor l in new_logs:\n    print(f\"処理: {l['id']} {l['msg']}\")\nif new_logs:\n    save_last_id(max(l[\"id\"] for l in new_logs))\nprint(f\"処理件数: {len(new_logs)}\")\n\nlogs2 = logs + [{\"id\": 104, \"msg\": \"D\"}]\nlast_id2 = load_last_id()\nnew_logs2 = [l for l in logs2 if l[\"id\"] > last_id2]\nfor l in new_logs2:\n    print(f\"処理: {l['id']} {l['msg']}\")\nprint(f\"処理件数: {len(new_logs2)}\")\n\nincremental処理（差分処理）とは、「前回どこまで処理したか」を記録しておき、次回はその続きだけを処理する方式です。バッチ処理のたびに全件を洗い替えると、データが増えるほど処理時間が伸びていきますが、差分だけを処理すればデータ量に関わらず高速に保てます。この問題ではファイル（state.json）に最後に処理したIDを保存し、次回の実行時にそのIDより大きいものだけを対象にすることで実現しています。2回目の実行ではlogs2に101〜103が含まれていても、last_id（103）より大きい104だけが新規として処理される点がポイントです。\n\nよくある間違い:\n・save_last_idを呼ぶタイミングを処理の前にしてしまい、処理が失敗した場合でも「処理済み」として記録されてしまう（本来は処理が成功したことを確認してから保存すべきだが、今回は簡略化のため成功前提で保存している）\n・new_logsが空（新しいログが無い）場合にmax()を呼んでValueErrorになる（今回はif new_logs:で空チェックをしてから呼んでいる）",
    hint: "last_id = load_last_id() の後、[l for l in logs if l[\"id\"] > last_id] で新しいログだけを抽出します。処理後は save_last_id(max(...)) で最大IDを保存します。",
    answerCode: "import json, os\n\nlogs = [{\"id\": 101, \"msg\": \"A\"}, {\"id\": 102, \"msg\": \"B\"}, {\"id\": 103, \"msg\": \"C\"}]\nSTATE_FILE = \"state.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_last_id():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)[\"last_id\"]\n    return 0\n\ndef save_last_id(last_id):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump({\"last_id\": last_id}, f)\n\nlast_id = load_last_id()\nnew_logs = [l for l in logs if l[\"id\"] > last_id]\nfor l in new_logs:\n    print(f\"処理: {l['id']} {l['msg']}\")\nif new_logs:\n    save_last_id(max(l[\"id\"] for l in new_logs))\nprint(f\"処理件数: {len(new_logs)}\")\n\nlogs2 = logs + [{\"id\": 104, \"msg\": \"D\"}]\nlast_id2 = load_last_id()\nnew_logs2 = [l for l in logs2 if l[\"id\"] > last_id2]\nfor l in new_logs2:\n    print(f\"処理: {l['id']} {l['msg']}\")\nprint(f\"処理件数: {len(new_logs2)}\")"
  },
  {
    id: "a079",
    level: "advanced",
    category: "engineering",
    title: "【incremental処理総合】状態ファイル方式でログを2回に分けて処理し、重複処理が起きないことを確認する",
    description: "状態ファイルに「最後に処理したID」と「累計ERROR件数」の両方を記録し、ログを2回に分けて処理してください。各回で「今回処理: N件（新規ERROR: M件） 累計ERROR: K件」の形式で出力し、2回目では1回目に処理した分が重複処理されないことを確認できるようにしてください。\n\n期待される出力:\n今回処理: 3件（新規ERROR: 2件） 累計ERROR: 2件\n今回処理: 2件（新規ERROR: 1件） 累計ERROR: 3件",
    starterCode: "import json, os\n\nSTATE_FILE = \"state_error.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_state():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)\n    return {\"last_id\": 0, \"error_total\": 0}\n\ndef save_state(state):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump(state, f)\n\ndef process(logs):\n    # 1) load_stateで状態を読む 2) last_idより新しいログを抽出\n    # 3) 新規ログ中のERROR件数を数え、error_totalに加算\n    # 4) 状態を保存し、\"今回処理: N件（新規ERROR: M件） 累計ERROR: K件\"を出力\n    pass\n\nlogs1 = [\n    {\"id\": 1, \"level\": \"INFO\"},\n    {\"id\": 2, \"level\": \"ERROR\"},\n    {\"id\": 3, \"level\": \"ERROR\"},\n]\nprocess(logs1)\n\nlogs2 = logs1 + [\n    {\"id\": 4, \"level\": \"INFO\"},\n    {\"id\": 5, \"level\": \"ERROR\"},\n]\nprocess(logs2)\n",
    expectedOutput: "今回処理: 3件（新規ERROR: 2件） 累計ERROR: 2件\n今回処理: 2件（新規ERROR: 1件） 累計ERROR: 3件",
    explanation: "正解例:\n\nimport json, os\n\nSTATE_FILE = \"state_error.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_state():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)\n    return {\"last_id\": 0, \"error_total\": 0}\n\ndef save_state(state):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump(state, f)\n\ndef process(logs):\n    state = load_state()\n    new_logs = [l for l in logs if l[\"id\"] > state[\"last_id\"]]\n    new_errors = sum(1 for l in new_logs if l[\"level\"] == \"ERROR\")\n    state[\"error_total\"] += new_errors\n    if new_logs:\n        state[\"last_id\"] = max(l[\"id\"] for l in new_logs)\n    save_state(state)\n    print(f\"今回処理: {len(new_logs)}件（新規ERROR: {new_errors}件） 累計ERROR: {state['error_total']}件\")\n\nlogs1 = [\n    {\"id\": 1, \"level\": \"INFO\"},\n    {\"id\": 2, \"level\": \"ERROR\"},\n    {\"id\": 3, \"level\": \"ERROR\"},\n]\nprocess(logs1)\n\nlogs2 = logs1 + [\n    {\"id\": 4, \"level\": \"INFO\"},\n    {\"id\": 5, \"level\": \"ERROR\"},\n]\nprocess(logs2)\n\ni092では「最後に処理したID」だけを状態として持ちましたが、実務のログ監視ではさらに「累計のERROR件数」のような集計値も一緒に状態として持ち越したいことがよくあります。この問題ではstateという辞書にlast_idとerror_totalの両方を持たせ、毎回の処理で新規分だけerror_totalに加算しています。2回目の呼び出し（logs2）にはid=1〜3も含まれていますが、last_id（3）以下なのでnew_logsには含まれず、新規のid=4・5だけが処理されます。もし状態を持たずに毎回全件を数え直すと、ログが増えるたびに処理時間が伸びるだけでなく、同じERRORを何度もカウントしてアラートが誤って鳴り続けるといった問題が起きます。\n\nよくある間違い:\n・new_errorsを数える際にnew_logsではなくlogs全体に対してカウントしてしまい、過去分のERRORまで累計に加算してしまう（idでの絞り込み後のnew_logsに対して数える必要がある）\n・state[\"error_total\"] += new_errors を save_state の後に書いてしまい、加算前の値が保存されてしまう（加算してから保存する順序が重要）",
    hint: "process(logs)の中で、state = load_state() → new_logs抽出 → new_errors集計 → state[\"error_total\"]に加算 → last_id更新 → save_state(state) → print、の順に処理します。",
    answerCode: "import json, os\n\nSTATE_FILE = \"state_error.json\"\nif os.path.exists(STATE_FILE):\n    os.remove(STATE_FILE)  # 実行のたびに前回の続きから始まらないよう、まず状態ファイルをリセットする\n\ndef load_state():\n    if os.path.exists(STATE_FILE):\n        with open(STATE_FILE) as f:\n            return json.load(f)\n    return {\"last_id\": 0, \"error_total\": 0}\n\ndef save_state(state):\n    with open(STATE_FILE, \"w\") as f:\n        json.dump(state, f)\n\ndef process(logs):\n    state = load_state()\n    new_logs = [l for l in logs if l[\"id\"] > state[\"last_id\"]]\n    new_errors = sum(1 for l in new_logs if l[\"level\"] == \"ERROR\")\n    state[\"error_total\"] += new_errors\n    if new_logs:\n        state[\"last_id\"] = max(l[\"id\"] for l in new_logs)\n    save_state(state)\n    print(f\"今回処理: {len(new_logs)}件（新規ERROR: {new_errors}件） 累計ERROR: {state['error_total']}件\")\n\nlogs1 = [\n    {\"id\": 1, \"level\": \"INFO\"},\n    {\"id\": 2, \"level\": \"ERROR\"},\n    {\"id\": 3, \"level\": \"ERROR\"},\n]\nprocess(logs1)\n\nlogs2 = logs1 + [\n    {\"id\": 4, \"level\": \"INFO\"},\n    {\"id\": 5, \"level\": \"ERROR\"},\n]\nprocess(logs2)"
  },
  {
    id: "i093",
    level: "intermediate",
    category: "engineering",
    title: "【datetime】strptime で文字列を日付にする",
    description: "ログの1行は「日付 時刻 レベル メッセージ」という形式の文字列です。datetime.strptime を使って日付と時刻の部分をdatetimeオブジェクトに変換し、「年月日」の形式（例: 2026年7月1日）で出力してください。\n\n期待される出力:\n2026年7月1日\n2026年7月2日",
    starterCode: "from datetime import datetime\n\nlogs = [\n    \"2026-07-01 09:15:00 INFO start\",\n    \"2026-07-02 14:30:00 ERROR failed\",\n]\n# 各ログを split(\" \", 3) で 日付・時刻・レベル・メッセージ に分解し、\n# datetime.strptime で日付時刻をパースして「年月日」の形式で出力してください\n",
    expectedOutput: "2026年7月1日\n2026年7月2日",
    explanation: "正解例:\n\nfrom datetime import datetime\n\nlogs = [\n    \"2026-07-01 09:15:00 INFO start\",\n    \"2026-07-02 14:30:00 ERROR failed\",\n]\nfor log in logs:\n    date_str, time_str, level, msg = log.split(\" \", 3)\n    dt = datetime.strptime(f\"{date_str} {time_str}\", \"%Y-%m-%d %H:%M:%S\")\n    print(f\"{dt.year}年{dt.month}月{dt.day}日\")\n\nログファイルの日時はほとんどの場合ただの文字列として記録されています。文字列のままでは「この日付より後か」「何日差があるか」といった計算ができないため、datetime.strptime(文字列, 書式)でPythonのdatetimeオブジェクトに変換します。書式の%Y%m%d%H%M%Sはそれぞれ4桁の年・2桁の月・2桁の日・2桁の時・分・秒に対応し、実際の文字列の並び方と一致させる必要があります。変換後はdt.year・dt.month・dt.dayのように属性として年月日を個別に取り出せます。ログ解析・日次バッチ処理では、この「文字列→datetime」への変換が最初の一歩になります。\n\nよくある間違い:\n・書式指定 %Y-%m-%d %H:%M:%S を実際の文字列の区切り方（ハイフン・コロン・スペース）と1文字でも違えて書き、ValueErrorになる\n・dt.month や dt.day がゼロ埋めされない数値（7月なら7、07ではない）であることを忘れ、期待する表示と桁数がずれる",
    hint: "log.split(\" \", 3) で [日付, 時刻, レベル, メッセージ] の4つに分けられます。datetime.strptime(f\"{日付} {時刻}\", \"%Y-%m-%d %H:%M:%S\") でdatetimeオブジェクトに変換できます。",
    answerCode: "from datetime import datetime\n\nlogs = [\n    \"2026-07-01 09:15:00 INFO start\",\n    \"2026-07-02 14:30:00 ERROR failed\",\n]\nfor log in logs:\n    date_str, time_str, level, msg = log.split(\" \", 3)\n    dt = datetime.strptime(f\"{date_str} {time_str}\", \"%Y-%m-%d %H:%M:%S\")\n    print(f\"{dt.year}年{dt.month}月{dt.day}日\")"
  },
  {
    id: "i094",
    level: "intermediate",
    category: "engineering",
    title: "【datetime】strftime で表示形式を変える",
    description: "日付の文字列（\"YYYY-MM-DD\"形式）を、datetime.strptimeで一度datetimeオブジェクトに変換し、strftimeを使って「YYYY年MM月DD日」の形式に変換して出力してください。\n\n期待される出力:\n2026年07月01日\n2026年12月25日",
    starterCode: "from datetime import datetime\n\ndates = [\"2026-07-01\", \"2026-12-25\"]\n# 各日付文字列をdatetime.strptimeでdatetimeオブジェクトに変換し、\n# strftimeで「YYYY年MM月DD日」の形式にして出力してください\n",
    expectedOutput: "2026年07月01日\n2026年12月25日",
    explanation: "正解例:\n\nfrom datetime import datetime\n\ndates = [\"2026-07-01\", \"2026-12-25\"]\nfor d in dates:\n    dt = datetime.strptime(d, \"%Y-%m-%d\")\n    print(dt.strftime(\"%Y年%m月%d日\"))\n\nstrptime（string parse time）が「文字列→datetime」の変換なのに対し、strftime（string format time）は逆に「datetime→好きな書式の文字列」への変換です。i093のdt.year・dt.monthのように属性を1つずつ組み立てる方法もありますが、strftime(書式)を使えば書式指定子（%Y %m %d など）を並べるだけで済み、桁揃え（07のようにゼロ埋め）も自動でやってくれます。ログの表示・レポートの見出し・ファイル名の生成など、日付を人間が読みやすい形式に変えたい場面でよく使います。\n\nよくある間違い:\n・strptime（読み込み）とstrftime（書き出し）の名前が似ていて、どちらがどちら向きか混同してしまう（覚え方: pはparse=読み込み、fはformat=書き出し）\n・%mや%dの代わりに桁揃えなしの指定子を使おうとしてしまう（OSによって挙動が異なるため、この教材では%Y%m%dのゼロ埋め指定に統一している）",
    hint: "datetime.strptime(文字列, \"%Y-%m-%d\") でdatetimeに変換した後、dt.strftime(\"%Y年%m月%d日\") で好きな形式の文字列にできます。",
    answerCode: "from datetime import datetime\n\ndates = [\"2026-07-01\", \"2026-12-25\"]\nfor d in dates:\n    dt = datetime.strptime(d, \"%Y-%m-%d\")\n    print(dt.strftime(\"%Y年%m月%d日\"))"
  },
  {
    id: "i095",
    level: "intermediate",
    category: "engineering",
    title: "【datetime】timedelta で日数計算",
    description: "2つの日付文字列の差（日数）と、ある日付からN日後の日付を計算してください。datetime同士の引き算はtimedeltaオブジェクトになり、.daysで日数を取り出せます。日付の加算にはtimedelta(days=N)を使います。\n\n期待される出力:\n差: 9日\n5日後: 2026-07-06",
    starterCode: "from datetime import datetime, timedelta\n\nd1 = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nd2 = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\n# d2とd1の差を日数で求めて \"差: N日\" の形式で出力してください\n# d1の5日後の日付を求めて \"5日後: YYYY-MM-DD\" の形式で出力してください\n",
    expectedOutput: "差: 9日\n5日後: 2026-07-06",
    explanation: "正解例:\n\nfrom datetime import datetime, timedelta\n\nd1 = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nd2 = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\ndiff = (d2 - d1).days\nprint(f\"差: {diff}日\")\nlater = d1 + timedelta(days=5)\nprint(f\"5日後: {later.strftime('%Y-%m-%d')}\")\n\ndatetime同士を引き算（d2 - d1）すると、結果は日数・秒数の差を表すtimedeltaというオブジェクトになります。.daysで「まる何日分の差か」を整数で取り出せます（a080で使うtotal_seconds()は秒単位で欲しいときに使います）。逆にdatetimeにtimedelta(days=N)を足すと「N日後の日付」が得られます。日次バッチ処理で「対象期間の終了日を算出する」「N日前までのデータだけを対象にする」といった場面はすべてこの計算の応用です。\n\nよくある間違い:\n・(d2 - d1).days ではなく (d2 - d1) をそのまま整数として使おうとしてTypeErrorになる（timedeltaはそのままでは数値として表示できず、.daysや.total_seconds()で明示的に取り出す必要がある）\n・N日後を計算するのに timedelta(N) と書いてしまう（第一引数はdaysではなくweeksなので、timedelta(days=N)と明示するのが安全）",
    hint: "(d2 - d1).days で日数差、d1 + timedelta(days=5) で5日後の日付が得られます。",
    answerCode: "from datetime import datetime, timedelta\n\nd1 = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nd2 = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\ndiff = (d2 - d1).days\nprint(f\"差: {diff}日\")\nlater = d1 + timedelta(days=5)\nprint(f\"5日後: {later.strftime('%Y-%m-%d')}\")"
  },
  {
    id: "a080",
    level: "advanced",
    category: "engineering",
    title: "【datetime】ログの開始・終了時刻から処理時間を計算",
    description: "各ジョブの開始時刻(start)・終了時刻(end)の文字列から処理時間を計算し、「ジョブ名: N秒」の形式で出力してください。差分のtimedeltaをtotal_seconds()で秒数に変換します。\n\n期待される出力:\njob_a: 750秒\njob_b: 45秒",
    starterCode: "from datetime import datetime\n\njobs = [\n    {\"name\": \"job_a\", \"start\": \"2026-07-01 09:00:00\", \"end\": \"2026-07-01 09:12:30\"},\n    {\"name\": \"job_b\", \"start\": \"2026-07-01 10:00:00\", \"end\": \"2026-07-01 10:00:45\"},\n]\n# 各ジョブについてstart・endをdatetimeにパースし、\n# (end - start).total_seconds() で処理時間（秒）を求めて出力してください\n",
    expectedOutput: "job_a: 750秒\njob_b: 45秒",
    explanation: "正解例:\n\nfrom datetime import datetime\n\njobs = [\n    {\"name\": \"job_a\", \"start\": \"2026-07-01 09:00:00\", \"end\": \"2026-07-01 09:12:30\"},\n    {\"name\": \"job_b\", \"start\": \"2026-07-01 10:00:00\", \"end\": \"2026-07-01 10:00:45\"},\n]\nfor job in jobs:\n    start = datetime.strptime(job[\"start\"], \"%Y-%m-%d %H:%M:%S\")\n    end = datetime.strptime(job[\"end\"], \"%Y-%m-%d %H:%M:%S\")\n    seconds = (end - start).total_seconds()\n    print(f\"{job['name']}: {seconds:.0f}秒\")\n\ni095では日数の差を.daysで取り出しましたが、秒単位・分単位のような細かい経過時間を知りたい場合はtotal_seconds()を使います。total_seconds()はtimedelta全体（日・時・分・秒すべて）をまとめて秒数の浮動小数で返してくれるため、時・分をまたぐ処理時間でも正しく計算できます。バッチ処理のジョブ実行時間の計測・SLA（対応時間の目標値）の監視など、DE実務でログの開始・終了時刻から所要時間を出す場面は非常に多く出てきます。\n\nよくある間違い:\n・.secondsを使ってしまう（.secondsは「日をまたいだ分の秒数の余り」しか返さず、1日以上かかった処理では正しい合計にならない。合計秒数が欲しい場合は必ずtotal_seconds()を使う）\n・total_seconds()の返り値が浮動小数点数であることを忘れ、そのままprintして750.0のような表示になってしまう（:.0fで整数表示に丸めている）",
    hint: "(end - start).total_seconds() で経過時間を秒数（浮動小数）で取得できます。整数で表示したい場合は f\"{seconds:.0f}\" のように書式指定します。",
    answerCode: "from datetime import datetime\n\njobs = [\n    {\"name\": \"job_a\", \"start\": \"2026-07-01 09:00:00\", \"end\": \"2026-07-01 09:12:30\"},\n    {\"name\": \"job_b\", \"start\": \"2026-07-01 10:00:00\", \"end\": \"2026-07-01 10:00:45\"},\n]\nfor job in jobs:\n    start = datetime.strptime(job[\"start\"], \"%Y-%m-%d %H:%M:%S\")\n    end = datetime.strptime(job[\"end\"], \"%Y-%m-%d %H:%M:%S\")\n    seconds = (end - start).total_seconds()\n    print(f\"{job['name']}: {seconds:.0f}秒\")"
  },
  {
    id: "a081",
    level: "advanced",
    category: "engineering",
    title: "【datetime】期間フィルタ",
    description: "各レコードの日付(date)が、指定した開始日〜終了日の範囲内（両端含む）にあるものだけを抽出し、件数と合計金額(amount)を出力してください。datetimeオブジェクト同士は <= や >= で比較できます。\n\n期待される出力:\n件数: 3\n合計: 6500",
    starterCode: "from datetime import datetime\n\nrecords = [\n    {\"id\": 1, \"date\": \"2026-06-28\", \"amount\": 1000},\n    {\"id\": 2, \"date\": \"2026-07-01\", \"amount\": 2000},\n    {\"id\": 3, \"date\": \"2026-07-05\", \"amount\": 1500},\n    {\"id\": 4, \"date\": \"2026-07-10\", \"amount\": 3000},\n    {\"id\": 5, \"date\": \"2026-07-15\", \"amount\": 500},\n]\nstart = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nend = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\n# 各レコードのdateをdatetimeに変換し、start <= d <= end の範囲内のものだけ集計してください\n# \"件数: N\" と \"合計: M\" の2行を出力してください\n",
    expectedOutput: "件数: 3\n合計: 6500",
    explanation: "正解例:\n\nfrom datetime import datetime\n\nrecords = [\n    {\"id\": 1, \"date\": \"2026-06-28\", \"amount\": 1000},\n    {\"id\": 2, \"date\": \"2026-07-01\", \"amount\": 2000},\n    {\"id\": 3, \"date\": \"2026-07-05\", \"amount\": 1500},\n    {\"id\": 4, \"date\": \"2026-07-10\", \"amount\": 3000},\n    {\"id\": 5, \"date\": \"2026-07-15\", \"amount\": 500},\n]\nstart = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nend = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\n\ncount = 0\ntotal = 0\nfor r in records:\n    d = datetime.strptime(r[\"date\"], \"%Y-%m-%d\")\n    if start <= d <= end:\n        count += 1\n        total += r[\"amount\"]\nprint(f\"件数: {count}\")\nprint(f\"合計: {total}\")\n\ndatetimeオブジェクトは日付として正しく大小比較できるため、start <= d <= end のように書くだけで「開始日以降・終了日以前」という範囲判定ができます（文字列のままの日付比較でも一見動きそうに見えますが、桁数の異なる表記が混ざると正しく比較できなくなるため、必ずdatetimeに変換してから比較するのが安全です）。日次バッチ処理の「今月分だけ集計する」「指定期間のレポートを作る」といった要件は、まさにこの期間フィルタの応用です。\n\nよくある間違い:\n・startとendのどちらかだけの片側比較（d >= start だけ）にしてしまい、終了日より後のデータまで含めてしまう\n・日付を文字列のまま比較してしまい、桁数の異なる表記（\"2026-7-1\"と\"2026-07-01\"など）が混在するデータで正しく判定できない",
    hint: "datetime.strptime(r[\"date\"], \"%Y-%m-%d\") でdatetimeに変換したうえで、start <= d <= end のように比較します。",
    answerCode: "from datetime import datetime\n\nrecords = [\n    {\"id\": 1, \"date\": \"2026-06-28\", \"amount\": 1000},\n    {\"id\": 2, \"date\": \"2026-07-01\", \"amount\": 2000},\n    {\"id\": 3, \"date\": \"2026-07-05\", \"amount\": 1500},\n    {\"id\": 4, \"date\": \"2026-07-10\", \"amount\": 3000},\n    {\"id\": 5, \"date\": \"2026-07-15\", \"amount\": 500},\n]\nstart = datetime.strptime(\"2026-07-01\", \"%Y-%m-%d\")\nend = datetime.strptime(\"2026-07-10\", \"%Y-%m-%d\")\n\ncount = 0\ntotal = 0\nfor r in records:\n    d = datetime.strptime(r[\"date\"], \"%Y-%m-%d\")\n    if start <= d <= end:\n        count += 1\n        total += r[\"amount\"]\nprint(f\"件数: {count}\")\nprint(f\"合計: {total}\")"
  },
  {
    id: "i096",
    level: "intermediate",
    category: "engineering",
    title: "【collections】Counter で頻度集計",
    description: "リストの中の要素の出現回数を数え、最も多く出現するものから順に上位2件を「要素名: 回数」の形式で出力してください。collections.Counterのmost_common()を使います。\n\n期待される出力:\napple: 3\nbanana: 2",
    starterCode: "from collections import Counter\n\nwords = [\"apple\", \"banana\", \"apple\", \"orange\", \"apple\", \"banana\", \"grape\"]\n# Counter(words) で頻度を数え、most_common(2) で上位2件を \"要素: 回数\" の形式で出力してください\n",
    expectedOutput: "apple: 3\nbanana: 2",
    explanation: "正解例:\n\nfrom collections import Counter\n\nwords = [\"apple\", \"banana\", \"apple\", \"orange\", \"apple\", \"banana\", \"grape\"]\ncounter = Counter(words)\nfor word, count in counter.most_common(2):\n    print(f\"{word}: {count}\")\n\nCounter(リスト)は、リストの中の各要素が何回出現したかを自動で数えて辞書のように扱えるオブジェクトを作ります。counter[\"apple\"]のようにキーでアクセスして件数を取り出すこともできますが、「多い順に並べたい」ときはmost_common()が便利です。引数なしのmost_common()は全件を多い順に、most_common(N)は上位N件だけをタプルのリスト（要素, 件数）で返します。アクセスログでよく踏まれたページを調べる・エラーログで頻発するエラー種別を調べるなど、集計＋ランキング表示はDE実務で頻出のパターンです。\n\nよくある間違い:\n・普通の辞書で手動でカウントを実装しようとして、初期値0の設定（dict.get(key, 0) + 1）を書き忘れてKeyErrorになる（Counterならその処理が不要）\n・most_common()の返り値がリストであることを忘れ、counter.most_common(2)[0]のようにインデックスでアクセスしようとして意図と違う結果になる（for文でタプルを展開して使うのが基本）",
    hint: "Counter(words).most_common(2) で「(要素, 件数)」のタプルが多い順に2件返ります。for word, count in ... で展開できます。",
    answerCode: "from collections import Counter\n\nwords = [\"apple\", \"banana\", \"apple\", \"orange\", \"apple\", \"banana\", \"grape\"]\ncounter = Counter(words)\nfor word, count in counter.most_common(2):\n    print(f\"{word}: {count}\")"
  },
  {
    id: "i097",
    level: "intermediate",
    category: "engineering",
    title: "【collections】defaultdict(list) でカテゴリ別にまとめる",
    description: "アイテムのリストを、category（カテゴリ）ごとにname（名前）のリストへまとめてください。defaultdict(list)を使うと、キーが存在しない場合の初期化を省略できます。カテゴリ名の昇順で出力してください。\n\n期待される出力:\nfruit: ['apple', 'banana', 'grape']\nvegetable: ['carrot', 'onion']",
    starterCode: "from collections import defaultdict\n\nitems = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n    {\"category\": \"fruit\", \"name\": \"grape\"},\n]\n# defaultdict(list)を使い、category別にnameのリストへまとめてください\n# カテゴリ名の昇順（sorted）で \"category: [name, ...]\" の形式で出力してください\n",
    expectedOutput: "fruit: ['apple', 'banana', 'grape']\nvegetable: ['carrot', 'onion']",
    explanation: "正解例:\n\nfrom collections import defaultdict\n\nitems = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n    {\"category\": \"fruit\", \"name\": \"grape\"},\n]\ngroups = defaultdict(list)\nfor item in items:\n    groups[item[\"category\"]].append(item[\"name\"])\n\nfor category in sorted(groups):\n    print(f\"{category}: {groups[category]}\")\n\n普通の辞書でgroups[category].append(...)を書くと、そのcategoryキーが初めて出てきたときにKeyErrorになってしまいます（先にgroups[category] = []と初期化しておく必要がある）。defaultdict(list)は「キーが存在しなければ自動的に空リストを作ってから使う」という辞書で、この初期化処理を省略できます。SQLのGROUP BYやpandasのgroupbyと同じ「あるキーでグルーピングする」操作を、標準ライブラリだけで素直に書けるのがdefaultdictの強みです。\n\nよくある間違い:\n・defaultdict(list)のlistを呼び出し形（list()）で書いてしまい、defaultdict(list())とTypeErrorになる（渡すのは「呼び出すと初期値を返す関数」自体であり、list（カッコなし）が正しい）\n・出力順がdictの挿入順（fruit, vegetable, fruitの登場順）になることを期待し、sorted()を忘れて順序が不安定になる",
    hint: "groups = defaultdict(list) としてから groups[item[\"category\"]].append(item[\"name\"]) でOKです。出力時は for category in sorted(groups): とします。",
    answerCode: "from collections import defaultdict\n\nitems = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n    {\"category\": \"fruit\", \"name\": \"grape\"},\n]\ngroups = defaultdict(list)\nfor item in items:\n    groups[item[\"category\"]].append(item[\"name\"])\n\nfor category in sorted(groups):\n    print(f\"{category}: {groups[category]}\")"
  },
  {
    id: "i098",
    level: "intermediate",
    category: "engineering",
    title: "【itertools】groupby で連続データをまとめる",
    description: "レコードのリストをcategoryでグルーピングしてください。itertools.groupbyは「連続している」要素だけをまとめるため、groupbyを使う前に必ずcategoryで事前にソートしておく必要があります（ソートを忘れると同じカテゴリでも別グループに分かれてしまいます）。\n\n期待される出力:\nfruit: ['apple', 'banana']\nvegetable: ['carrot', 'onion']",
    starterCode: "from itertools import groupby\n\nrecords = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n]\n# まずsorted(records, key=...)でcategory順に並べ替えてから、\n# groupby(並べ替えた後のリスト, key=...)でグルーピングし、\n# \"category: [name, ...]\" の形式で出力してください\n",
    expectedOutput: "fruit: ['apple', 'banana']\nvegetable: ['carrot', 'onion']",
    explanation: "正解例:\n\nfrom itertools import groupby\n\nrecords = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n]\nrecords_sorted = sorted(records, key=lambda r: r[\"category\"])\nfor category, group in groupby(records_sorted, key=lambda r: r[\"category\"]):\n    names = [r[\"name\"] for r in group]\n    print(f\"{category}: {names}\")\n\nitertools.groupbyは名前だけ見るとSQLのGROUP BYやpandasのgroupbyと同じことをしてくれそうに見えますが、実際は「隣り合った要素が同じキーのときだけ」1つのグループにまとめる、という全く違う動き方をします。この問題の元データはfruit, vegetable, fruit, vegetableの順に並んでおり、ソートせずにgroupbyへ渡すと「fruit（1件）→vegetable（1件）→fruit（1件）→vegetable（1件）」という4つのバラバラなグループになってしまいます。必ずsorted(..., key=同じキー関数)で事前に整列させてから渡すのが、groupbyを使う上での鉄則です。\n\nよくある間違い:\n・事前ソートを忘れてgroupbyに直接渡してしまい、本来2グループになるはずが4グループ以上に分かれてしまう（この教材で最も注意してほしい落とし穴）\n・groupbyが返すgroup（イテレータ）は、for文の中で一度しか読み出せないことを知らずに2回list()しようとして2回目が空になる（今回はnamesのリスト内包表記で1回だけ読み出している）",
    hint: "sorted(records, key=lambda r: r[\"category\"]) で並べ替えてから groupby(..., key=lambda r: r[\"category\"]) に渡します。groupの中身は [r[\"name\"] for r in group] のようにリスト内包表記で取り出せます。",
    answerCode: "from itertools import groupby\n\nrecords = [\n    {\"category\": \"fruit\", \"name\": \"apple\"},\n    {\"category\": \"vegetable\", \"name\": \"carrot\"},\n    {\"category\": \"fruit\", \"name\": \"banana\"},\n    {\"category\": \"vegetable\", \"name\": \"onion\"},\n]\nrecords_sorted = sorted(records, key=lambda r: r[\"category\"])\nfor category, group in groupby(records_sorted, key=lambda r: r[\"category\"]):\n    names = [r[\"name\"] for r in group]\n    print(f\"{category}: {names}\")"
  },
  {
    id: "a082",
    level: "advanced",
    category: "engineering",
    title: "【collections】Counter同士の加算で複数ログの集計をマージ",
    description: "2つの日にそれぞれ集計されたログレベル別件数（Counter）を1つに合算し、ERROR・WARN・INFOの順で「レベル: 件数」を出力してください。CounterはCounter同士を+で足し合わせることができます。\n\n期待される出力:\nERROR: 5\nWARN: 6\nINFO: 35",
    starterCode: "from collections import Counter\n\nlog_day1 = Counter({\"ERROR\": 3, \"WARN\": 5, \"INFO\": 20})\nlog_day2 = Counter({\"ERROR\": 2, \"WARN\": 1, \"INFO\": 15})\n# log_day1とlog_day2を+で合算し、[\"ERROR\", \"WARN\", \"INFO\"]の順で\n# \"レベル: 件数\" の形式で出力してください\n",
    expectedOutput: "ERROR: 5\nWARN: 6\nINFO: 35",
    explanation: "正解例:\n\nfrom collections import Counter\n\nlog_day1 = Counter({\"ERROR\": 3, \"WARN\": 5, \"INFO\": 20})\nlog_day2 = Counter({\"ERROR\": 2, \"WARN\": 1, \"INFO\": 15})\n\ntotal = log_day1 + log_day2\nfor level in [\"ERROR\", \"WARN\", \"INFO\"]:\n    print(f\"{level}: {total[level]}\")\n\nCounterは普通の辞書と違い、+演算子でキーごとの値を足し合わせた新しいCounterを作れます（同じキーがあれば加算、片方にしかないキーはそのまま引き継がれます）。日ごと・サーバーごとに集計したログの件数をあとから1つにまとめたい場合、log_day1.update(log_day2)のようにミュータブルに合算する方法もありますが、+を使うと元のCounterを変更せずに新しい合計を得られるため、集計結果を使い回したい場面では扱いやすくなります。複数のバッチジョブの結果を後からマージする、といった処理の基本形です。\n\nよくある間違い:\n・辞書の | 演算子（マージ演算子）と混同してしまう（辞書の|は後勝ちで値を上書きするだけで、値同士を加算はしない。Counter同士の加算には+を使う）\n・total[level]の代わりにtotal.get(level)のように書いてしまい、存在しないレベルの場合の挙動を気にしすぎる（Counterはキーが無くても0を返す設計になっているため、通常の辞書のようにgetを使う必要はない）",
    hint: "log_day1 + log_day2 でキーごとに件数が合算されたCounterが得られます。total[\"ERROR\"]のようにキーでアクセスできます。",
    answerCode: "from collections import Counter\n\nlog_day1 = Counter({\"ERROR\": 3, \"WARN\": 5, \"INFO\": 20})\nlog_day2 = Counter({\"ERROR\": 2, \"WARN\": 1, \"INFO\": 15})\n\ntotal = log_day1 + log_day2\nfor level in [\"ERROR\", \"WARN\", \"INFO\"]:\n    print(f\"{level}: {total[level]}\")"
  },
  {
    id: "a083",
    level: "advanced",
    category: "engineering",
    title: "【総合】datetime × Counter でエラーログの日別集計レポート",
    description: "ログの各行は「日付 時刻 レベル メッセージ」の形式です。ERRORレベルのログだけを日付ごとに集計し、日付の昇順で「日付: N件」の形式で出力してください。datetime.strptimeでのパースとCounterでの集計を組み合わせます。\n\n期待される出力:\n2026-07-01: 1件\n2026-07-02: 2件\n2026-07-03: 1件",
    starterCode: "from datetime import datetime\nfrom collections import Counter\n\nlogs = [\n    \"2026-07-02 10:00:00 ERROR disk full\",\n    \"2026-07-01 09:00:00 ERROR timeout\",\n    \"2026-07-02 11:30:00 ERROR timeout\",\n    \"2026-07-01 15:00:00 INFO ok\",\n    \"2026-07-03 08:00:00 ERROR timeout\",\n]\n# 各ログをsplit(\" \", 3)で分解し、日付・時刻をdatetime.strptimeでパースしてください\n# レベルがERRORのものだけ、strftime(\"%Y-%m-%d\")した日付をキーにCounterで集計してください\n# 日付の昇順（sorted）で \"日付: N件\" の形式で出力してください\n",
    expectedOutput: "2026-07-01: 1件\n2026-07-02: 2件\n2026-07-03: 1件",
    explanation: "正解例:\n\nfrom datetime import datetime\nfrom collections import Counter\n\nlogs = [\n    \"2026-07-02 10:00:00 ERROR disk full\",\n    \"2026-07-01 09:00:00 ERROR timeout\",\n    \"2026-07-02 11:30:00 ERROR timeout\",\n    \"2026-07-01 15:00:00 INFO ok\",\n    \"2026-07-03 08:00:00 ERROR timeout\",\n]\ncounter = Counter()\nfor log in logs:\n    date_str, time_str, level, msg = log.split(\" \", 3)\n    dt = datetime.strptime(f\"{date_str} {time_str}\", \"%Y-%m-%d %H:%M:%S\")\n    if level == \"ERROR\":\n        counter[dt.strftime(\"%Y-%m-%d\")] += 1\n\nfor date in sorted(counter):\n    print(f\"{date}: {counter[date]}件\")\n\nここまでのdatetime（strptime・strftime）とCounter（頻度集計）を組み合わせた総合問題です。ログはi093と同じ要領でstrptimeによりdatetimeへ変換し、strftime(\"%Y-%m-%d\")で「時刻を切り捨てた日付だけの文字列」をキーとして作り、Counterに積み上げています。こうすることで、元のログの並び順がバラバラでも、日付ごとの件数に正しく集計できます。最後にsorted(counter)でキー（日付文字列）を昇順に並べており、TASK-013のSQL問題で学んだ「ORDER BYで出力順を確定させる」のと同じ原則です（辞書やCounterの反復順は挿入順であり、日付順とは限らないため）。障害対応でよくある「日ごとのエラー件数推移レポート」は、まさにこの形の集計です。\n\nよくある間違い:\n・counter[dt.strftime(\"%Y-%m-%d\")]の代わりにdate_str（split直後の文字列）をそのままキーにしてしまい、日付の妥当性チェックを経ていない生の文字列に頼ってしまう（今回のデータでは同じ結果になるが、書式が微妙に揺れるログが混在する場合はstrptime/strftimeを経由した方が安全）\n・sorted(counter)を忘れ、Counterに最初に登場した日付順（2026-07-02が先に来る元データの並び順）で出力してしまい、期待する日付の昇順と一致しない",
    hint: "log.split(\" \", 3) → datetime.strptimeでパース → level==\"ERROR\"なら counter[dt.strftime(\"%Y-%m-%d\")] += 1。最後に for date in sorted(counter): で日付順に出力します。",
    answerCode: "from datetime import datetime\nfrom collections import Counter\n\nlogs = [\n    \"2026-07-02 10:00:00 ERROR disk full\",\n    \"2026-07-01 09:00:00 ERROR timeout\",\n    \"2026-07-02 11:30:00 ERROR timeout\",\n    \"2026-07-01 15:00:00 INFO ok\",\n    \"2026-07-03 08:00:00 ERROR timeout\",\n]\ncounter = Counter()\nfor log in logs:\n    date_str, time_str, level, msg = log.split(\" \", 3)\n    dt = datetime.strptime(f\"{date_str} {time_str}\", \"%Y-%m-%d %H:%M:%S\")\n    if level == \"ERROR\":\n        counter[dt.strftime(\"%Y-%m-%d\")] += 1\n\nfor date in sorted(counter):\n    print(f\"{date}: {counter[date]}件\")"
  },
];
