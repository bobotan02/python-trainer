...
- TASK-015で追加したpandas/numpy遅延ロードの実ブラウザでの動作（`import pandas`を含むコードの実行。初回ロードに数秒かかる想定・ステータスバーの「📦 pandas を読み込んでいます…」表示・2回目以降のロード時間短縮）
...
- feature/task015ブランチで作業。実ブラウザでの`import pandas`を含むコードの実行確認は未実施（ユーザー側で実施予定）
