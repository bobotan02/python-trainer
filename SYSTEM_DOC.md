...
- TASK-013で追加したSQL・正規表現25問の実ブラウザでの表示・判定（Pyodide上のsqlite3・reモジュールでの実行を含む。Python3ローカル環境では全問検証済みだが、Pyodide環境（Python 3.13）での実行は未確認）
...
- **`problems.js`のみ変更**（末尾に25問追加。既存150問は無変更。`git diff`で確認済み）:
...
- 検証: 全25問の正解例をPython3（sqlite 3.46.1）で実行しexpectedOutputと完全一致を確認（25/25。付録B）。全175問のフィールド機械検証合格（付録C・id重複なし）。全175問のanswerCode単体実行監査 175/175 PASS（付録A）。`node --check problems.js` 合格。`git diff`でproblems.jsが末尾追記のみであることを確認（既存150問は無変更）
- SYSTEM_DOC.md 9節（問題セット）の数値・テーマ表を更新（計175問: beginner 21 / intermediate 83 / advanced 71、engineering 48→73）。feature/task013ブランチで作業
